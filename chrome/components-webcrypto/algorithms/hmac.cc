...
const char* GetJwkHmacAlgorithmName(blink::WebCryptoAlgorithmId hash) {
  switch (hash) {
    ...
    case blink::kWebCryptoAlgorithmIdSha256:
      std::cout << "hmac GetJwkHmacAlgorithmName" << std::endl;
      return "HS256";
...
class HmacImplementation : public AlgorithmImplementation {
 public:
  ...
  Status GenerateKey(const blink::WebCryptoAlgorithm& algorithm,
    ...
    std::cout << "generate hmac key" << std::endl;
  ...
  Status ImportKey(blink::WebCryptoKeyFormat format,
    ...
    std::cout << "import hmac key" << std::endl;
    switch (format) {
  ...
  Status ImportKeyJwk(const CryptoData& key_data,
                      ...
                      blink::WebCryptoKeyUsageMask usages,
                      blink::WebCryptoKey* key) const {
    std::cout << "import hmac key raw" << std::endl;
    Status status = CheckKeyCreationUsages(kAllKeyUsages, usages);
    if (status.IsError())
      return status;
  ...
  Status Sign(const blink::WebCryptoAlgorithm& algorithm,
              ...
              const CryptoData& data,
              std::vector<uint8_t>* buffer) const override {
    std::cout << "sign hmac key" << std::endl;
    const blink::WebCryptoAlgorithm& hash =
  ...
  Status Verify(const blink::WebCryptoAlgorithm& algorithm,
    ...
    std::cout << "verify hmac key" << std::endl;
