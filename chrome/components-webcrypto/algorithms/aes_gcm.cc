...
const EVP_AEAD* GetAesGcmAlgorithmFromKeySize(size_t key_size_bytes) {
  switch (key_size_bytes) {
    case 16:
      std::cout << "EVP_aead_aes_128_gcm" << std::endl;
...
Status AesGcmEncryptDecrypt(EncryptOrDecrypt mode,
  ...
  // std::vector<uint8_t>* fakeBuffer = nullptr;
  ...
  // SymEncryptionDecryption - cb
  // if (mode == ENCRYPT) {
  //   return TPMDoEncryptOrDecrypt(true, raw_key, data, tag_length_bits / 8, CryptoData(params->Iv()),
  //       CryptoData(params->OptionalAdditionalData()),
  //       GetAesGcmAlgorithmFromKeySize(raw_key.size()), buffer);
  // } else {
  //   return TPMDoEncryptOrDecrypt(false, raw_key, data, tag_length_bits / 8, CryptoData(params->Iv()),
  //       CryptoData(params->OptionalAdditionalData()),
  //       GetAesGcmAlgorithmFromKeySize(raw_key.size()), buffer);
  // }
