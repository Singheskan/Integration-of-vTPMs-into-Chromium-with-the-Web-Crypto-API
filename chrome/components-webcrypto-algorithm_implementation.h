...
// crypto.subtle.tpm...() dispatches to these functions-------------------------------------------------------------------------

    virtual Status DigestTest(const blink::WebCryptoAlgorithm& algorithm,
                        const CryptoData& data,
                        std::vector<uint8_t>* buffer) const;

    virtual Status TPMInit();

    virtual Status TPMGetRandom(
        std::vector<uint8_t>* buffer);

    virtual Status TPMCreatePrimary();

    virtual Status TPMCreate();

    virtual Status TPMEncrypt();

    virtual Status TPMDecrypt();

    virtual Status TPMSign();

    virtual Status TPMVerify();

    virtual Status TPMFlushContext();
